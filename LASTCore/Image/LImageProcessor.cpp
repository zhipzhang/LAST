...
void LImageProcessor::Compute_Concentration(LRDL1TelEvent &dl1televent)
{
    ...
    double psi = dl1televent.GetPsi();
    double length = dl1televent.GetLength();
    double width  = dl1televent.GetWidth();
    double cog_size = 0;
    double core_size = 0;
    std::vector<int> cog_pixels;  // pixels in one pixel distance away from the core
    std::vector<int> core_pixels; // pixels in the ellipse
    tel_geometry->FillCogPixels(x, y, focal_length, cog_pixels);
    tel_geometry->FillCorePixels(x, y, focal_length,psi, length, width, core_pixels);

    for(auto ipix : cog_pixels)
    {
        cog_size += cleaned_image[ipix];
    }
    for(auto ipix:core_pixels)
    {
        core_size += cleaned_image[ipix];
    }
